...
typedef enum {
    BALL_CONTACT_NONE = 0,
    BALL_CONTACT_FLOOR,
    BALL_CONTACT_LEFT_WALL,
    BALL_CONTACT_RIGHT_WALL,
    BALL_CONTACT_FAR_WALL,
    BALL_CONTACT_NEAR_WALL
} BallContact;
...
typedef struct {
    ...
    u8  spriteSlot;
    u8  shadowSlot;
    bool eliminatorArt;       /* compact PAL3 ball used when PAL3 also holds kits */
    u8 contactKind;           /* BallContact generated by the latest loose update */
    s16 contactVX, contactVY; /* incoming vector before a boundary reverses it */
} Ball;
...
/* Converts an airborne player hit into a short, directional loose-ball
 * rebound so the ball clears the victim instead of dropping under their feet. */
void ball_startHitBounce(Ball *b, s16 x, s16 y);
/* Drops a carried/expired ball at a fixed point with only a tiny spin kick. */
