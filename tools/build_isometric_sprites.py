...
def main():
    ...
    arrays = []
    pose_canvases = {}
    preview = Image.new("RGB", (4 * 40, 40), (24, 40, 72))
    for pose, box in zip(POSES, boxes):
        ...
        arrays.append((pose, encode_tiles(canvas)))
        ...
        for y in range(32):
            for x in range(32):
                if canvas[y][x]:
                    rp[x, y] = PALETTE[canvas[y][x]]
        preview.paste(rendered, (4 + POSES.index(pose) * 40, 4))

    lines = ["/* Generated by tools/build_isometric_sprites.py. */", ""]
    for pose, tiles in arrays:
        lines.append(f"static const u32 tile_iso_{pose}[16][8] = {{")
        for rows in tiles:
            values = ", ".join(f"0x{value:08x}" for value in rows)
            lines.append(f"    {{ {values} }},")
        lines.extend(["};", ""])
    for pose in POSES:
        lines.append(f"static const u32 tile_iso_far_{pose}[9][8] = {{")
        for rows in encode_far(pose_canvases[pose]):
            values = ", ".join(f"0x{value:08x}" for value in rows)
            lines.append(f"    {{ {values} }},")
        lines.extend(["};", ""])
    OUTPUT.write_text("\n".join(lines), encoding="ascii")
    preview.resize((640, 160), Image.Resampling.NEAREST).save(PREVIEW)
